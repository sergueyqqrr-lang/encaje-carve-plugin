...
void EncajeCarveAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    // varios plugins cargados, las instancias no se estan comunicando.
    juce::String peerText = peers <= 1
        ? "Sola en el grupo: no hay con quien repartir espacio"
        : ("Repartiendo espacio con " + juce::String (peers - 1) + " pista(s) mas");
    ...
    drawBandMeters (g, area.removeFromTop (200));
}
...
void EncajeCarveAudioProcessorEditor::drawBandMeters (juce::Graphics& g, juce::Rectangle<int> area)
{
    ...
    for (int b = 0; b < numBands; ++b)
    {
        ...
        if (fight)
        {
            g.setColour (mine ? juce::Colour (0xff6fa98a) : juce::Colour (0xffc1443c));
        ...
        juce::String tag = mine     ? juce::String ("AL FRENTE")
                         : beingCut ? juce::String (cutDb, 1) + " dB"
                                    : juce::String ("-");
        g.setColour (mine     ? juce::Colour (0xff6fa98a)
                   : beingCut ? juce::Colour (0xffc1443c)
                              : juce::Colour (0xff5f5a51));
...
{
    auto area = getLocalBounds().reduced (18);
    area.removeFromTop (58 + 8 + 200 + 12);

    auto knobRow = area.removeFromTop (104);
    const int third = knobRow.getWidth() / 3;

    auto a1 = knobRow.removeFromLeft (third).reduced (4);
    carveLabel.setBounds (a1.removeFromBottom (15));
    carveSlider.setBounds (a1);

    auto a2 = knobRow.removeFromLeft (third).reduced (4);
    maxCutLabel.setBounds (a2.removeFromBottom (15));
    maxCutSlider.setBounds (a2);

    auto a3 = knobRow.reduced (4);
