...
void EncajeCarveAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    for (int b = 0; b < encaje::kNumBands; ++b)
    {
        ...
        {
            // Brecha de derecho: cuanto menos me corresponde esta banda frente
            // al dueno. Un intruso claro se aparta a fondo.
            float gap = (v.leaderClaim - v.myClaim)
                          / (v.leaderClaim > 1.0e-6f ? v.leaderClaim : 1.0f);
            gap = juce::jlimit (0.0f, 1.0f, gap / 0.55f);

            // Cuanto estorbo de verdad ahi.
            float presence = juce::jlimit (0.0f, 1.0f,
                                v.share / (v.leaderShare > 1.0e-6f ? v.leaderShare : 1.0f));

            // Piso de accion: si hay un choque real (dos pistas ocupando la
            // misma banda y una es la dueña), el recorte nunca debe quedar por
            // debajo de lo audible. Sin esto, casos legitimos como bombo/bajo
            // daban 0,7 dB y el plugin parecia apagado.
        ...
        // makePeakFilter reserva memoria, y hacerlo en cada bloque en el hilo
        // de audio provoca cortes. Solo se recalcula cuando el cambio ya es
        // perceptible.
